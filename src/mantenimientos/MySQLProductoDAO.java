...
public class MySQLProductoDAO implements ProductoDAO {
	...
	public ArrayList<ProductoDTO> listadoProductos() {
		...
		try {
			...
			String consulta="SELECT p.cod_producto, p.nom_producto, p.precio_producto, p.stock_producto, c.cod_cat, c.nom_categoria, p.eliminar , p.imagen"
			...
			while(rs.next()){
				...
				objProducto.setImagen(rs.getString(8));
	...
	public ArrayList<ProductoDTO> listadoProductos(String categ) {
		...
		try {
			...
			while(rs.next()){
				...
				objProducto.setImagen(rs.getString(7));
				listaProducto.add(objProducto);
			}
		}catch (Exception e) {
			// TODO: handle exception
	...
	public int modificarProducto(ProductoDTO c) {
		...
		try {
			con =  MySQLConexion8.getConexion();
			String sql = "update producto set nom_producto=?,precio_producto=?,stock_producto=?,cod_cat=?,imagen=? where cod_producto=?";
			...
			pst.setString(6, c.getCod_producto());
