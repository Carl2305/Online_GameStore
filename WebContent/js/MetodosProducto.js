...
var sendProducto = function()
{
	if(boolIMG===true){
		$.ajax({
			...
			success: function(data){
				...
				if(data=='1'){
					...
					LoadTablaProducts();
					boolIMG=false;
				}else{SweetAlertMessage('Registro Fallido !','','error',true,1500); URLImage=""; boolIMG=false;}
			},
			error:function(e){
	    ...
	    SweetAlertMessage("Error", "Debes seleccionar una imagen o archivo para mostrar en la web", "error", "Continuar", 2000);
		return false;
	}	
}
	...
	$.ajax({
		...
		success: function(d){
			if(d=='1'){
				...
				$('#imgPreview').attr('src', "images/UploadLogo.jpg");
				LoadTablaProducts();
			}else{SweetAlertMessage('Actualización Fallida !','','error',true,1500); URLImage="";}
		},
